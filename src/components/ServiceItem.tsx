...
const getProperties = (slug: string) => {};

const ServiceItem: FC<serviceDataType> = (props) => {
  const { title, description, slug } = props;
  const { selectedService } = useSelector((store: RootState) => store.service);
  const dispatch = useDispatch();
  return (
    <div
      onClick={() => dispatch(selectService(props))}
      className={`relative cursor-pointer group bg-white block p-4 rounded-lg h-[220px] border-2 ${
        selectedService.slug === slug ? "border-emerald-500 shadow-lg" : ""
      }  hover:border-emerald-500 hover:shadow-lg`}
    >
      {selectedService.slug === slug && (
